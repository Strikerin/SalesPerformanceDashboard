...
def process_data(df):
    ...
    if 'company' in data.columns:
        ...
        customer_stats['profit_margin'] = customer_stats['profit'] / customer_stats['revenue'] * 100
...
def import_data_component():
    """Streamlit component for importing Excel data."""
    st.subheader("Import Work History Data")
    
    # File uploader
    uploaded_file = st.file_uploader("Upload Work History Excel", type=["xlsx"])
        ...
        with st.spinner("Processing data..."):
            try:
                ...
                if df is not None and not df.empty:
                    # Process the data
                    yearly_summary, quarterly_summary, customer_data, workcenter_data, part_data = process_data(df)
                    
                    # Store in session state
                    st.session_state['df'] = df
                    st.session_state['yearly_summary'] = yearly_summary
                    st.session_state['quarterly_summary'] = quarterly_summary
                    st.session_state['customer_data'] = customer_data
                    st.session_state['workcenter_data'] = workcenter_data
                    st.session_state['part_data'] = part_data
