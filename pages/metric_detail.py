...
# Page configuration
st.set_page_config(
    page_title="Metric Detail View",
    page_icon="🔍",
    layout="wide",
    initial_sidebar_state="expanded"
)

# Initialize session state for the selected metric
...
if not st.session_state.get('data_loaded', False):
    st.warning("No data loaded. Please go back to the main dashboard and upload your work history data.")
        ...
        if selected_metric == "overrun_analysis":
            ...
            if not overrun_jobs.empty:
                ...
                with tab1:
                    ...
                    top_overruns = overrun_jobs.sort_values('overrun_hours', ascending=False).head(20)
                    
                    # Create chart
                    if 'job_id' in top_overruns.columns:
                        fig = go.Figure()
                        
                        fig.add_trace(go.Bar(
                            y=top_overruns['job_id'].head(10),
                            x=top_overruns['planned_hours'].head(10),
                            name='Planned Hours',
                            orientation='h',
                            marker_color='#1e40af'
                        ))
                        ...
                        fig.add_trace(go.Bar(
                            y=top_overruns['job_id'].head(10),
                            x=top_overruns['actual_hours'].head(10),
                            name='Actual Hours',
                            orientation='h',
                            marker_color='#dc2626'
                        ))
                    ...
                    available_cols = [col for col in display_cols if col in top_overruns.columns]
                ...
                with tab3:
                    ...
                    if 'date' in overrun_jobs.columns:
                        ...
                        fig.add_trace(go.Scatter(
                            x=monthly_overruns['month'],
                            y=monthly_overruns['overrun_hours'],
                            mode='lines+markers',
                            name='Overrun Hours',
                        ...
                        fig.add_trace(go.Scatter(
                            x=monthly_overruns['month'],
                            y=monthly_overruns['overrun_percent'],
                            mode='lines+markers',
                            name='Overrun %',
            ...
            if 'company' in df.columns:
                ...
                with tab1:
                    ...
                    # Visualize top and bottom 10 customers by profit margin
                    fig = go.Figure()
                    
                    # Top 10
                    top10 = margin_sorted.head(10)
                    fig.add_trace(go.Bar(
                        x=top10['company'],
                        y=top10['profit_margin'],
                        marker_color='#4caf50',
                        name='Top 10',
                        text=top10['profit_margin'].apply(lambda x: f"{x:.1f}%"),
                        textposition='outside'
                    ))
                ...
                with tab2:
                    ...
                    # Visualize top 10 customers by total profit
                    fig = go.Figure()
                    
                    fig.add_trace(go.Bar(
                        x=profit_sorted.head(10)['company'],
                        y=profit_sorted.head(10)['profit'],
                        marker_color='#1e40af',
                        text=profit_sorted.head(10)['profit'].apply(lambda x: format_money(x)),
                        textposition='outside'
                    ))
                    ...
                    other_profit = profit_sorted.iloc[10:]['profit'].sum() if len(profit_sorted) > 10 else 0
                    
                    labels = list(profit_sorted.head(10)['company'])
                    if other_profit > 0:
                        labels.append('Others')
                    
                    values = list(profit_sorted.head(10)['profit'])
                    if other_profit > 0:
                        values.append(other_profit)
                    ...
                    with col2:
                        ...
                        top10_percent = top10_profit / total_profit * 100
            ...
            if 'work_center' in df.columns:
                ...
                with tab1:
                    ...
                    # Use only top 15 work centers for the chart
                    top_wc = wc_data.head(15)
                    ...
                    fig.update_layout(
                        ...
                        yaxis=dict(
                            range=[0, max(150, top_wc['utilization'].max() * 1.1)]
                        )
                    )
                    ...
                    fig.add_shape(
                        ...
                        y0=0,
                        x1=100,
                        y1=wc_data['utilization'].value_counts().max() * 1.1,
                        line=dict(
                            color="red",
                            width=2,
                            dash="dash",
                ...
                with tab2:
                    ...
                    labels = list(top_wc['work_center'])
                    others_actual = wc_data.iloc[10:]['actual_hours'].sum() if len(wc_data) > 10 else 0
                    
                    if others_actual > 0:
                        labels.append('Others')
                    
                    values = list(top_wc['actual_hours'])
                    if others_actual > 0:
                        values.append(others_actual)
                    ...
                    fig.update_layout(
                        ...
                        title="Distribution of Actual Hours"
                    )
