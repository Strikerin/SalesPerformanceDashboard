...
# Page configuration
st.set_page_config(
    page_title="Yearly Work History View",
    page_icon="📅",
    layout="wide",
    initial_sidebar_state="expanded"
)

# Initialize session state for the current year view
        ...
        if selected_year:
            ...
            if year_df.empty:
                ...
                fig.add_trace(go.Bar(
                    ...
                    y=quarterly_data['planned_hours'],
                    name='Planned Hours',
                    marker_color='#1e40af'
                ))
                ...
                with tab1:
                    if 'work_center' in year_df.columns:
                        ...
                        fig.add_trace(go.Bar(
                            x=workcenter_data['work_center'],
                            y=workcenter_data['utilization'],
                            marker_color=workcenter_data['utilization'].apply(
                                lambda x: '#4caf50' if x <= 100 else '#f44336'
                            ),
                            text=workcenter_data['utilization'].apply(lambda x: f"{x:.1f}%"),
                        ...
                        fig.update_layout(
                            ...
                            yaxis=dict(
                                range=[0, max(150, workcenter_data['utilization'].max() * 1.1)]
                            )
                        )
                ...
                with tab2:
                    if 'company' in year_df.columns:
                        ...
                        fig = px.bar(
                            ...
                            text=customer_data.head(10)['profit_margin'].apply(lambda x: f"{x:.1f}%")
                        )
